...
import { getNearestPharmacies } from "@/lib/api";
...
export default function NearestPharmacyPage() {
  ...
  const { data: pharmacies, isLoading } = useQuery({
    ...
    queryFn: () => getNearestPharmacies(coords!.lat, coords!.lng),
    enabled: !!coords,
  });
  ...
  const requestLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        setGeoError("Konum izni verilmedi. En yakın eczaneyi gösterebilmemiz için tarayıcıya izin vermeniz gerekiyor.");
        setRequesting(false);
      }
    );
  };
        ...
        <div className="mx-auto max-w-2xl">
          ...
          {pharmacies && pharmacies.length > 0 && (
            <div className="grid gap-4">
              {pharmacies.map((p, i) => (
                <PharmacyCard key={p.name} pharmacy={p} index={i} />
              ))}
            </div>
          )}
        </div>
